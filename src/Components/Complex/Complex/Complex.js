...
import {architecture, safety, engineering, transport, landscaping, infrastructure} from '../Svg/'
import {CSSTransition} from 'react-transition-group';

const namesCategories = [
    {
        name: 'Архитектура',
        svg: <img className={complex.imgSvg} src={architecture} alt="architecture"/>,
        link: 'Architecture',
    ...
    },
    {
        ...
        svg: <img className={complex.imgSvg} src={landscaping} alt="landscaping"/>,
        link: 'Landscaping',
    ...
    {
        name: "Безопасность",
        svg: <img className={complex.imgSvg} src={safety} alt="safety"/>,
        link: 'Safety',
    ...
    {
        name: "Инженерия",
        svg: <img className={complex.imgSvg} src={engineering} alt="engineering"/>,
        link: 'Engineering',
        text: 'Оригинальная архитектура жилого комплекса бизнес-класса «Первомайская» формирует современный стиль жизни',
        path: '/Complex/3'
    },
    {
        name: "Инфраструктура",
        svg: <img className={complex.imgSvg} src={infrastructure} alt="infrastructure"/>,
        link: 'Infrastructure',
    ...
    },
    {
        ...
        svg: <img className={complex.imgSvg} src={transport} alt="transport"/>,
        link: 'Transport',
        ...
        <div className={complex.linkDiv}>
            <ul className={complex.ul}>
                {namesCategories.map((item, index) => {
                    return <li key={index} className={complex.liCategories}>
                                <NavLink className={complex.spanCategories}
                                         activeClassName={complex.spanCategoriesActive}
                                         to={item.path}
                                         exact>{item.name}
                                </NavLink>
                           </li>
                ...
                <Route key={path} exact path={path}>
                    {({match}) => {
                        return <CSSTransition
                            ...
                            unmountOnExit
                        >
                            <span className={complex.spanNumber}>{index + 1}</span>
                        </CSSTransition>
                    }}
                </Route>
                ...
                <Route key={path} exact path={path}>
                    {({match}) => (
                        <CSSTransition
                            ...
                            timeout={500}
                            classNames={{
                                enter: index < indexMassive.current ? complex.imgEnterDown : complex.imgEnter,
                                exitActive: complex.imgExit
                            }}
                            onEnter={() => {
                                indexMassive.current = index
                            }}
                            unmountOnExit
                        >
